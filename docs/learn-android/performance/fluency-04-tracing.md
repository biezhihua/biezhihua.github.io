...
profiling（分析）是用于测量和分析软件程序或系统行为的技术。它涉及在程序执行过程中收集各种指标的数据，如 CPU 使用率、内存使用率和 I/O 操作。然后，收集到的数据可以用于识别性能瓶颈和优化区域。通常将分析（profiling）与跟踪（tracing）结合使用，以提供系统行为的全面图像。
...
跟踪（tracing）和分析（profiling）是性能分析中两种广泛使用的技术。Perfetto是一套开源工具，结合了跟踪（tracing）和分析（profiling）技术，为用户提供强大的系统分析功能。

### Tracing - 跟踪

跟踪（tracing）涉及收集关于系统执行的高度详细数据。单次连续会话的记录被称为跟踪文件或简称为跟踪（a trace file or trace）。

跟踪（tracing）包含足够的详细的信息以完全重建事件的时间线。它们通常包括诸如调度程序上下文切换、线程唤醒、系统调用等低级别的内核事件。有了“正确”的跟踪，就不需要再重现性能错误，因为跟踪提供了所有必要的上下文。

被认为重要的程序代码也会进行插桩。这种插桩跟踪了程序随时间的运行情况（例如，正在运行哪些函数，每次调用花费了多长时间）以及执行的上下文（例如，函数调用的参数是什么，为什么要运行某个函数）。

跟踪（tracing）中的详细信息水平使得在除了最简单的情况之外，直接阅读跟踪就像读取日志文件一样变得不切实际。相反，用户会使用跟踪分析库（trace analysis）和跟踪查看器（trace viwers）的组合。跟踪分析库为用户提供了以编程方式提取和总结跟踪事件（trace events）的方法。跟踪查看器（trace viewers）将跟踪中的事件可（events in a trace）视化为时间线，使用户能够以图形方式查看系统在一段时间内的操作。
...
小插曲：理论上，跟踪框架（tracing frameworks）的功能也足以充当日志记录系统。但是，在实践中，两者的利用方式有所不同，因此它们往往是分开的。
...
- 为什么使用跟踪（tracing）来重构事件时间线时，分析（profiling）会给出使用最多资源的精确代码行？

##### When to use profiling over tracing - 何时使用分析而不是跟踪

跟踪（tracing）无法捕获极高频事件（例如每个函数调用）的执行，因此分析（profiling）工具填补了这一空白。通过抽样，它们可以大幅减少存储的信息量。统计分析工具的统计性质很少是问题；抽样算法专门设计用于捕获高度代表实际资源使用情况的数据。

附注：一些非常专业的跟踪工具可以捕获每个函数调用（例如 magic-trace），但它们每秒输出几十亿字节的数据，使它们对除了调查微小代码片段之外的任何事情都不切实际。它们的开销通常也比通用跟踪工具高。

##### When to use tracing over profiling - 何时使用跟踪而不是分析

虽然分析工具（profiler）可以提供资源使用的调用栈，但它们缺乏有关为什么会发生这种情况的信息。例如，为什么函数 foo() 调用了这么多次 malloc？它们只会说 foo() 在 Y 次 malloc 调用中分配了 X 个字节。跟踪（tracing）在提供这种准确上下文方面非常出色：应用程序的插装代码和低级内核事件共同提供了深入的见解：为什么代码首先被运行在这里。
...
Perfetto 提供了一个 C++ SDK，可以用于程序以发出跟踪事件。该 SDK 设计为非常低开销，并以合并形式的一个 .cc 文件和一个 .h 文件进行分发，易于集成到任何构建系统中。
...
接下来，文章介绍了分析（profiling）和跟踪（tracing）之间的区别分析（profiling）能够提供系统中资源使用的函数调用堆栈，而跟踪（tracing）则能够提供函数调用堆栈以及执行时的上下文信息。
